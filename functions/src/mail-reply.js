...
const inviaRispostaEmailPratica=onCall({region:'us-central1',timeoutSeconds:120,memory:'256MiB',secrets:[GMAIL_TOKEN_ENCRYPTION_KEY]},async request=>{
 ...
 const to=addresses(email.direzione==='inviata'?email.destinatari:(email.replyTo?.length?email.replyTo:email.mittente))[0];
 if(!to)throw new HttpsError('failed-precondition','Destinatario originale mancante.');
 ...
 const hash=createHash('sha256').update(JSON.stringify({emailId,text,to,from,subject})).digest('hex');
 ...
 if(prior?.status==='sent')return {ok:true,sent:true,to,messageId,alreadySent:true};
 ...
 try{
  ...
  const info=await transport.sendMail({from,to,subject,text:text.trim(),messageId,inReplyTo:email.messageId||undefined,references:email.messageId?[email.messageId]:undefined});
  if(!info.accepted?.length)throw new Error('Il server non ha accettato il destinatario.');
 ...
 try{
  ...
  await ref.collection('email_timeline').doc(id).set({messageId,oggetto:subject,testo:text.trim(),html:'',bodyVersion:3,direzione:'inviata',mittente:[from],destinatari:[to],replyTo:[],allegati:[],data:admin.firestore.FieldValue.serverTimestamp(),autoAssociata:false,matchMethod:'manual',banca:email.banca||null,consultantUid:uid,casella:from,inReplyTo:email.messageId||null});
 }catch(_){timelineWarning='Email inviata; la copia in conversazione sarà disponibile alla sincronizzazione.';}
 return {ok:true,sent:true,to,messageId,timelineWarning};
});
