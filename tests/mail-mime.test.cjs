...
let update,readOnly=false,loggedOut=false;
...
class ImapFlow{async connect(){}async getMailboxLock(folder,options){readOnly=options.readOnly;return {release(){}};}async fetchOne(){return {source:raw};}async logout(){loggedOut=true;}}
...
(async()=>{
 ...
 console.log('PASS real MIME parser: original HTML restored, richer HTML text sent to AI, old analysis invalidated, IMAP read-only, connection closed');
