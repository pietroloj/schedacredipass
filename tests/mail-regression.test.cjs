...
if(n==='./mail-recipients')return require(root+'/functions/src/mail-recipients');
...
const sendRequest={auth:{uid:'owner'},data:{practiceId:'molino',emailId:'good',requestId:'test-operation-123456',text:'Risposta modificata direttamente'}};
const sent=await reply.inviaRispostaEmailPratica(sendRequest);
check('send uses edited text and original recipient',()=>{assert(sent.sent);assert.equal(sentPayload.text,sendRequest.data.text);assert.deepEqual([...sentPayload.to],['bank@example.com']);});
...
const recipientSend={auth:{uid:'owner'},data:{practiceId:'molino',emailId:'good',requestId:'reply-all-test-123456',text:'Risposta a tutti',to:'bank@example.com,owner@example.com',cc:'colleague@example.com,bank@example.com',replyAll:true}};
await reply.inviaRispostaEmailPratica(recipientSend);
check('SMTP delivers edited To and CC without self or duplicates',()=>{assert.deepEqual([...sentPayload.to],['bank@example.com']);assert.deepEqual([...sentPayload.cc],['colleague@example.com']);});
const alias=await engine.resolveTimelineEmail(ref('pratiche_mutuo/molino'),'email_good',records.get('pratiche_mutuo/molino'));
check('legacy timeline prefix resolves to actual email',()=>assert.equal(alias.id,'good'));
await assert.rejects(engine.resolveTimelineEmail(ref('pratiche_mutuo/molino'),'missing',records.get('pratiche_mutuo/molino')),/non è più associata/);
console.log(`Backend: ${count+4} checks passed`);
