...
async function boot(search='',deny=false){
 ...
 let denied=deny;
 const fn={httpsCallable:name=>async args=>{calls.push({name,args});if(name==='statoCollegamentoGmail')return {data:{connected:true,email:'test@example.com',lastSyncOk:false,lastError:'Errore simulato',lastSyncAt:'2026-09-18'}};if(denied)throw Error('Accesso negato');return {data:{practice:data,thread:[{id:args.threadAfter?'second':'first',oggetto:'Richiesta <img>',mittente:['bank@example.com'],data:'2026-09-18'}],threadNext:args.threadAfter?null:'first'}}}};
...
test('home presents actual requests and honest Gmail failure',async()=>{const x=await boot();assert.match(x.node('view').innerHTML,/Il prossimo passo/);assert.match(x.node('view').innerHTML,/Avanzamento delle tue pratiche/);assert.match(x.node('syncState').innerHTML,/Ultimo tentativo non riuscito/);assert.match(x.node('syncState').innerHTML,/Errore simulato/)});
test('common consultant menu has separate operational entry and archive',()=>{const auth=fs.readFileSync(path.join(__dirname,'../main/auth-guard.js'),'utf8');assert.equal((auth.match(/href="\/main\/area-operativa.html"/g)||[]).length,1);assert.match(auth,/href="\/main\/ricerca-clienti.html"/);const archive=fs.readFileSync(path.join(__dirname,'../main/ricerca-clienti.html'),'utf8');assert(!archive.includes('Apri Area operativa'));});
